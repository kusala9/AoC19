...
public class mz
{
    ...
    public String valat(int x,int y)
    {
        ...
        if (v.containsKey(pos))
        {
            ret+="T=" + v.get(pos);
            ns+=v.get(pos);
            if (otherwire.containsKey(pos))
            {
                ret+=" O=" + otherwire.get(pos);
                ns+=otherwire.get(pos);
            }
        }
        if (ns<minsteps) minsteps=ns;

        return ret;
    }
    public int man()
    {
        int x =  Math.abs(pos_x) + Math.abs(pos_y);
        if (x<minman) minman=x;
        return x;
    }
    ...
    private HashMap<String,Integer> otherwire = new HashMap<>();
    public void setOtherWire(HashMap<String,Integer> w1)
    {
        v.clear();
        pos_x=0;
        pos_y=0;
    ...
    {
        String pos = x +"," + y;
        if (v.containsKey(pos)) return true;
        else return false;
    }
    public boolean cross(int x,int y)
    {
        String pos = x + "," + y;
        if (v.containsKey(pos) && otherwire.containsKey(pos))
        {
            //othersteps = otherwire.get(pos);
            return true;
        }
        else
            return false;
    ...
    public void add(int x,int y)
    {
        String pos = x + "," + y;
        v.put(pos,steps);
        //System.out.println(steps + ": Adding -> " + pos);
    }

    public boolean goTo(String ins)
    {
        boolean ret = false;

        String dir = ins.substring(0,1);
        int n = Integer.parseInt(ins.substring(1));
        //System.out.println(steps + ": " + ins + ": -> " + pos_x + ":" + pos_y);

        for (int i=0;i<n;i++)
        {
            steps++;
            // just in case
            if (dir.compareTo("U")==0) pos_y++;
            if (dir.compareTo("D")==0) pos_y--;
            if (dir.compareTo("L")==0) pos_x--;
            if (dir.compareTo("R")==0) pos_x++;
            add(pos_x,pos_y);
            if (hasVisited(pos_x,pos_y))
            {
                //System.out.println(" -- been too..." + pos_x + "," + pos_y);
            }
            if (cross(pos_x, pos_y))
            {
                System.out.println(steps + " -- Crossing point..." + pos_x + "," + pos_y + "-> " + man() + " " + valat(pos_x,pos_y) );
            }
